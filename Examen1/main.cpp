...
void circularShift(char*, unsigned int, unsigned int, bool);
...
void circularShift(char* bitmap, unsigned int positionByte, unsigned int disp_amount, bool disp_dir) 
{
	char tempByte;
	tempByte = bitmap[positionByte];
	bitmap[positionByte] = bitmap[positionByte] | (0 << disp_amount);
	tempByte |= (0 >> (8 - disp_amount));
	bitmap[positionByte] = bitmap[positionByte] + tempByte;
	//for (int i = posicionInicial; i < (positionByte * 8) + 8; i++)
	//{
	//	if (i == nBlock)
	//	{
	//		bitmap[posicionByte] = bitMap[posicionByte] | (1 << x);
